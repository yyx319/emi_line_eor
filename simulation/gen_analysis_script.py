...
op_a2 = np.arange(31, 70, 1) # snapshots for which we perform analysis 
...
with open('ramses_first_look.sh', 'w') as f:
    ...
    for snapnum in op_a2:
        ix = np.where(snapnum_a==snapnum )[0][0]
        main_halo_pos = mainhalo_pos_a[ix]
        print(mode)
        f.write('# %d \n'%snapnum )
        snapnum = int(snapnum)
        ...
        xmin, ymin, zmin = gas_halo_pos - size/2
        xmax, ymax, zmax = gas_halo_pos + size/2
